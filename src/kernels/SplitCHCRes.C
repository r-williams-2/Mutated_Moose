...
<<<<<<< HEAD
  ...
  residual += (1.0 / _no_mols[_qp]) * 0.5 * _kappa[_qp] * _grad_u[_qp] * _grad_test[_i][_qp]; // This is the coupled gradients term for (1/n)*k*grad(c_i)*grad(test), where c_i is the primary concentration.
  ...
  // }
  residual += (1.0 / _no_mols[_qp]) * 0.5 * _kappa[_qp] * _grad_coupled[_qp] * _grad_test[_i][_qp]; // This is the coupled gradients term for (1/n)*k*grad(c_j)*grad(test), where c_j is the secondary concentration.
