...
validParams<SplitCHCRes>()
{
  ...
  params.addRequiredCoupledVar("w", "chem poten"); // Syntax for coupled variable for split CH (i.e. the variational derivative)
...
SplitCHCRes::computeQpResidual()
{
  ...
  residual += (1.0 / _no_mols[_qp]) * _kappa[_qp] * _grad_u[_qp] * _grad_test[_i][_qp]; // This is the coupled gradients term for (1/n)*k*grad(c_i)*grad(test), where c_i is the primary concentration.
  ...
  // }
  residual += (1.0 / _no_mols[_qp]) * _kappa[_qp] * _grad_coupled[_qp] * _grad_test[_i][_qp]; // This is the coupled gradients term for (1/n)*k*grad(c_j)*grad(test), where c_j is the secondary concentration.
...
SplitCHCRes::computeQpJacobian()
{
  ...
  jacobian += (1.0 / _no_mols[_qp]) * _kappa[_qp] * _grad_phi[_j][_qp] * _grad_test[_i][_qp];
